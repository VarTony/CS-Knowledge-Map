...
Чем больше диски, тем сложнее работать с сортировкой.  
Объём ОЗУ раньше исчислялся килобайтами, а данные на вторичных устройствах занимали мегабайты —  
и базовые алгоритмы вроде `quicksort` просто не могли загрузить весь объём данных в оперативную память.  

В итоге, в 70-х родился классический алгоритм сортировки — **TPMMS (Two-Phase Multiway Merge Sort)**.  
Он был разработан как ответ на ограничение по памяти:  
если не получается обработать всё разом — нужно **разбивать на куски и слить**.

---

## Идея

TPMMS лежит в архитектурной основе многих современных СУБД.  
Когда в PostgreSQL, MySQL или Oracle выполняется `ORDER BY` по объемной таблице,  
